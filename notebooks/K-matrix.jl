...
theme(:boxed; colorbar=false, clim=(-1, 1), lab="",
...
begin
	...
	# Simulated observed data (with noise)
...
md"""
## Naive Fit: Sum of Two Breit-Wigners + Background
This approach is often used but may not capture the correct analytic properties of the scattering amplitude.
"""

# ╔═╡ b8629b10-4093-4cad-8805-2547e44b9f8a
function naive_model(p, e)
    m1, g1, m2, g2, b0, b1 = p
    return breit_wigner(m1, g1, e) .+ breit_wigner(m2, g2, e) .+ b0 .* exp.(-b1 .* (e .- 0.5))
end

# ╔═╡ 815d1098-c59d-4ec7-8721-c576e45dfea5
begin
	loss_naive(p) = sum((naive_model(p, e) .- y_obs).^2)
	
	p0 = [0.95, 0.1, 1.55, 0.1, 0.4, 2.0]
	res_naive = optimize(loss_naive, p0)
	pfit_naive = Optim.minimizer(res_naive)
end
...
# ╔═╡ 4d0deaae-95ed-4a7e-b2aa-16a95029f3f7
md"""
## K-Matrix Formalism
The K-matrix allows a proper treatment of unitarity in multi-channel scattering.
We consider a 2-pole K-matrix:

$K(E) = \sum_{i=1}^2 \frac{g_i^2}{m_i^2 - E^2}$

We relate it to the scattering amplitude through:

$T(E) = \frac{K(E)}{1 - i \rho(E) K(E)}$

Where $\rho(E) = \sqrt{1 - 4m^2/E^2}$ is the phase space factor (simplified).
"""

# ╔═╡ 40aa1a2f-d133-47d7-bf8a-8127c7e6c2c6
function rho(e)
    return sqrt.(1 .- (4 * 0.14^2) ./ e.^2)
end

# ╔═╡ c3cffb0b-4267-4494-a364-186b0b1e389f
function k_matrix(p, e)
    m1, g1, m2, g2 = p
    k = (g1^2) ./ (m1^2 .- e.^2) .+ (g2^2) ./ (m2^2 .- e.^2)
    return abs.(k ./ (1 .- im .* rho(e) .* k))
end

# ╔═╡ 14d1b444-7668-40d5-8f7b-8b8824008ec3
function k_model(p, e)
    m1, g1, m2, g2, b0, b1 = p
    return k_matrix((m1, g1, m2, g2), e) .+ b0 .* exp.(-b1 .* (e .- 0.5))
end

# ╔═╡ 122cb494-fd82-4ccf-87a6-7278597d0e09
begin
	loss_k(p) = sum((k_model(p, e) .- y_obs).^2)
	
	p0_k = [1.0, 0.1, 1.6, 0.1, 0.4, 2.0]
	res_k = optimize(loss_k, p0_k)
	pfit_k = Optim.minimizer(res_k)
end
...
# ╔═╡ 1a48aac7-fb1d-4c50-bff9-b741b6a36f16
md"""
### Extracted Parameters (K-matrix Fit)
- m₁ = $(round(pfit_k[1], digits=3))
- g₁ = $(round(pfit_k[2], digits=3))
- m₂ = $(round(pfit_k[3], digits=3))
- g₂ = $(round(pfit_k[4], digits=3))

These are the effective couplings in the Flatté-like parameterization.
"""

# ╔═╡ 15572419-d364-4516-b5d0-1012868d9165
md"""
---
